...
export function PortalClient({ bookingUrl, consultorio }: Props) {
  ...
  const qrUrl = bookingUrl
    ? `https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(bookingUrl)}`
    : null
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 220px", gap: 16, alignItems: "start" }}>
        ...
        <div className="card">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {[
              {
                n: "1",
                titulo: "WhatsApp personal o de consultorio",
                desc: "Pega el link en tu estado de WhatsApp o envíalo directamente a los pacientes que quieren agendar.",
              },
              {
                n: "2",
                titulo: "Instagram o redes sociales",
                desc: 'Agrega el link en tu bio de Instagram con el texto "Agenda tu cita aquí".',
              },
              {
                n: "3",
                titulo: "Tarjeta de presentación digital",
                desc: "Imprime el código QR (a la derecha) en tu tarjeta física o compártelo digitalmente.",
              },
              {
                n: "4",
                titulo: "Firma de correo",
                desc: "Agrega el link al pie de tus emails con el texto "Agenda en línea".",
              },
            ].map((item) => (
      ...
      <div className="stack">
        <div className="card" style={{ textAlign: "center" }}>
          ...
          {qrUrl && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <a
                href={qrUrl}
                download={`qr-${consultorio.slug ?? "agendamed"}.png`}
        ...
        <div className="card" style={{ background: "var(--c-surface-2)" }}>
          ...
          {consultorio.ciudad && (
            <div className="muted" style={{ fontSize: "var(--fs-xs)", marginTop: 4 }}>📍 {consultorio.ciudad}</div>
          )}
          {consultorio.telefono && (
            <div className="muted" style={{ fontSize: "var(--fs-xs)", marginTop: 4 }}>📞 {consultorio.telefono}</div>
          )}
        </div>
      </div>
    </div>
